...
if (!fs.existsSync("./credentials.json"))
    fs.writeFileSync(
        "./credentials.json",
        JSON.stringify(credentialsDefault, null, 4)
    );
...
async function main() {
    ...
    await db
        .collection("profileStore")
        .createIndex({ apis: 1 }, { partialFilterExpression: { apis: true } });
    ...
    await db
        .collection("members")
        .createIndex({ uuid: 1, profile_id: 1 }, { unique: true });
    ...
    for (const id in constants.item_tags) {
        await db
            .collection("items")
            .updateOne({ id }, { $set: { tag: constants.item_tags[id] } });
    }
    ...
    await db
        ...
        .createIndex({ profile_id: 1 }, { unique: true });
