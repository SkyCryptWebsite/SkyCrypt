document.addEventListener("DOMContentLoaded", function () {
    ...
    if (calculated.skin_data) {
        skinViewer = new skinview3d.SkinViewer({
            ...
            skinUrl:
                "/texture/" + calculated.skin_data.skinurl.split("/").pop(),
            capeUrl:
                "capeurl" in calculated.skin_data
                    ? "/texture/" +
                      calculated.skin_data.capeurl.split("/").pop()
    ...
    tippyInstance = tippy(".interactive-tooltip", {
        ...
        onTrigger(instance, event) {
            if (event.type == "click") dimmer.classList.add("show-dimmer");
        },
        onHide() {
    ...
    function isEnchanted(item) {
        ...
        if ("tag" in item && Array.isArray(item.tag.ench)) return true;
    ...
    function renderLore(text) {
        ...
        const matchingEnchants = constants.special_enchants.filter((a) =>
            output.includes(a)
        );
        ...
        for (const enchantment of matchingEnchants)
            output = output.replace(
                enchantment,
                `<span style='${specialColor.css}'>${enchantment}</span>`
            );
    ...
    function renderInventory(inventory, type) {
        ...
        let visibleInventory = document.querySelector(
        ...
        if (visibleInventory) {
            ...
            document
                .querySelector("#inventory_container")
                .removeChild(visibleInventory);
        }
        ...
        inventory.forEach(function (item) {
            if (Object.keys(item).length > 2) countSlotsUsed++;
        });
            ...
            default:
                if (type in calculated.bag_sizes)
                    inventory = inventory.slice(
                        0,
                        Math.max(countSlotsUsed - 1, calculated.bag_sizes[type])
                    );
        }
        ...
        inventory.forEach(function (item, index) {
            ...
            if (item.id) {
                ...
                inventoryItemIcon.className =
                    "piece-icon item-icon icon-" + item.id + "_" + item.Damage;

                if (item.texture_path) {
                    inventoryItemIcon.className += " custom-icon";
                    inventoryItemIcon.style.backgroundImage =
                        'url("' + item.texture_path + '")';
                }
        ...
        [].forEach.call(
            inventoryView.querySelectorAll(".item-icon.is-enchanted"),
            handleEnchanted
        );
        ...
        if (rect.top < 0 || rect.bottom > window.innerHeight)
            inventoryStatContainer.scrollIntoView({
                block: "nearest",
                behavior: "smooth",
            });
    }
    ...
    function showBackpack(item) {
        let activeInventory = document.querySelector(
            ".inventory-tab.active-inventory"
        );
    ...
    function fillLore(element) {
        ...
        if (element.hasAttribute("data-backpack-index")) {
            let backpack = all_items.filter(
                (a) =>
                    a.item_index ==
                    Number(element.getAttribute("data-backpack-index"))
            );
            ...
            item = backpack.containsItems.filter(
                (a) =>
                    a.item_index ==
                    Number(element.getAttribute("data-item-index"))
            );
        } else if (element.hasAttribute("data-item-index"))
            item = all_items.filter(
                (a) =>
                    a.item_index ==
                    Number(element.getAttribute("data-item-index"))
            );
            ...
            item = [
                currentBackpack.containsItems[
                    Number(element.getAttribute("data-backpack-item-index"))
                ],
            ];
        else if (element.hasAttribute("data-pet-index"))
            item = [
                calculated.pets[
                    parseInt(element.getAttribute("data-pet-index"))
                ],
            ];
        else if (element.hasAttribute("data-missing-pet-index"))
            item = [
                calculated.missingPets[
                    parseInt(element.getAttribute("data-missing-pet-index"))
                ],
            ];
        else if (element.hasAttribute("data-missing-talisman-index"))
            item = [
                calculated.missingTalismans[
                    parseInt(
                        element.getAttribute("data-missing-talisman-index")
                    )
                ],
            ];
        ...
        if (element.hasAttribute("data-item-index"))
            ...
            statsContent.setAttribute(
                "data-backpack-item-index",
                element.getAttribute("data-backpack-item-index")
            );
        else if (element.hasAttribute("data-pet-index"))
            statsContent.setAttribute(
                "data-backpack-item-index",
                element.getAttribute("data-pet-index")
            );

        itemName.className =
            "item-name " + "piece-" + (item.rarity || "common") + "-bg";
        ...
        if (item.texture_path) {
            ...
            itemIcon.className =
                "stats-piece-icon item-icon icon-" +
                item.id +
                "_" +
                item.Damage;
        }
        ...
        if (item.texture_pack) {
            const texturePack = extra.packs.filter(
                (a) => a.id == item.texture_pack
            )[0];
            ...
            packIcon.setAttribute(
                "src",
                item.texture_pack.base_path + "/pack.png"
            );
            ...
            packAuthor.innerHTML =
                "by <span>" + item.texture_pack.author + "</span>";
        ...
        if (Array.isArray(item.containsItems)) {
            ...
            item.containsItems.forEach((backpackItem, index) => {
                ...
                if (backpackItem.id) {
                    ...
                    let enchantedClass = isEnchanted(backpackItem)
                        ? "is-enchanted"
                        : "";

                    inventoryItemIcon.className =
                        "piece-icon item-icon " +
                        enchantedClass +
                        " icon-" +
                        backpackItem.id +
                        "_" +
            ...
            [].forEach.call(
                document.querySelectorAll(
                    ".contains-backpack .item-icon.is-enchanted"
                ),
                handleEnchanted
            );
    ...
    function showLore(element, _resize) {
        ...
        if (_resize != false) resize();
    }

    function closeLore() {
        let shownLore = document.querySelector(
            "#stats_content.show-stats, #stats_content.sticky-stats"
        );

        if (shownLore != null) {
            dimmer.classList.remove("show-dimmer");

            let stickyStatsPiece = document.querySelector(
                ".rich-item.sticky-stats"
            );
        ...
        if (openedWardrobe) openedWardrobe.classList.remove("wardrobe-opened");
    }
    ...
    function resize() {
        if (window.innerWidth <= 1570 && (oldWidth === null || oldWidth > 1570))
            document
                .getElementById("skin_display_mobile")
                .appendChild(skinViewer.domElement);

        if (window.innerWidth > 1570 && oldWidth <= 1570)
            document
                .getElementById("skin_display")
                .appendChild(skinViewer.domElement);
        ...
        if (skinViewer) {
            if (playerModel.offsetWidth / playerModel.offsetHeight < 0.6)
                skinViewer.setSize(
                    playerModel.offsetWidth,
                    playerModel.offsetWidth * 2
                );
            else
                skinViewer.setSize(
                    playerModel.offsetHeight / 2,
                    playerModel.offsetHeight
                );
        }
        ...
        if (rect.x)
            statsContent.style.left =
                rect.x - statsContent.offsetWidth - 10 + "px";
    ...
    [].forEach.call(
        document.querySelectorAll(".sub-extendable .stat-sub-header"),
        function (element) {
            element.addEventListener("click", function (e) {
                if (element.parentNode.classList.contains("sub-extended"))
                    element.parentNode.classList.remove("sub-extended");
                else element.parentNode.classList.add("sub-extended");
            });
        }
    );

    [].forEach.call(
        document.querySelectorAll(".sub-floor-extendable .stat-sub-header"),
        function (element) {
            element.addEventListener("click", function (e) {
                if (element.parentNode.classList.contains("sub-extended"))
                    element.parentNode.classList.remove("sub-extended");
    ...
    [].forEach.call(
        document.querySelectorAll(".stat-weapons .select-weapon"),
        function (element) {
            ...
            if (element.parentNode.hasAttribute("data-backpack-index")) {
                ...
                filterItems = items.weapons.filter(
                    (a) => !("backpackIndex" in a)
                );
            }
            ...
            element.addEventListener("click", function (e) {
                if (element.parentNode.classList.contains("piece-selected")) {
                    ...
                    document.querySelector(".stat-active-weapon").className =
                        "stat-value stat-active-weapon piece-common-fg";
                    document.querySelector(".stat-active-weapon").innerHTML =
                        "None";
                } else {
                    [].forEach.call(
                        document.querySelectorAll(".stat-weapons .piece"),
                        function (_element) {
                    ...
                    document.querySelector(".stat-active-weapon").className =
                        "stat-value stat-active-weapon piece-" +
                        item.rarity +
                        "-fg";
                    document.querySelector(".stat-active-weapon").innerHTML =
                        item.display_name;
                ...
                for (let stat in stats) {
                    if (stat == "sea_creature_chance") continue;

                    let element = document.querySelector(
                        ".basic-stat[data-stat=" + stat + "] .stat-value"
                    );
    ...
    [].forEach.call(
        document.querySelectorAll(".stat-fishing .select-rod"),
        function (element) {
            let itemId = element.parentNode.getAttribute("data-item-id");
            let filterItems;
            ...
            element.addEventListener("click", function (e) {
                if (element.parentNode.classList.contains("piece-selected")) {
                    ...
                    document.querySelector(".stat-active-rod").className =
                        "stat-value stat-active-rod piece-common-fg";
                    document.querySelector(".stat-active-rod").innerHTML =
                        "None";
                } else {
                    [].forEach.call(
                        document.querySelectorAll(".stat-fishing .piece"),
                        function (_element) {
                            _element.classList.remove("piece-selected");
                        }
                    );
                    ...
                    document.querySelector(".stat-active-rod").className =
                        "stat-value stat-active-rod piece-" +
                        item.rarity +
                        "-fg";
                    document.querySelector(".stat-active-rod").innerHTML =
                        item.display_name;
                ...
                let _element = document.querySelector(
                    ".basic-stat[data-stat=sea_creature_chance] .stat-value"
                );
    ...
    enchantedGlint.onload = function () {
        [].forEach.call(
            document.querySelectorAll(".item-icon.is-enchanted"),
            handleEnchanted
        );
    };
    ...
    [].forEach.call(
        ...
        function (element) {
            ...
            element.addEventListener("click", function () {
                if (element.classList.contains("active-inventory")) return;
    ...
    function bindLoreEvents(element) {
        element.addEventListener("mouseenter", function (e) {
            ...
            if (
                ...
                !element.parentNode.parentNode.classList.contains(
                    "wardrobe-opened"
                )
            )
                return;
        ...
        element.addEventListener("mousemove", function (e) {
            if (statsContent.classList.contains("sticky-stats")) return;
            ...
            if (left < 10) left = rect.x + 90;

            if (rect.x) statsContent.style.left = left + "px";

            let top = Math.max(
                70,
                Math.min(maxTop, e.clientY - statsContent.offsetHeight / 2)
            );
        ...
        const itemIndex = Number(
            element.parentNode.getAttribute("data-item-index")
        );
        ...
        element.addEventListener("click", function (e) {
            ...
            if (
                element.parentNode.parentNode.classList.contains("wardrobe-set")
            )
            ...
            if (e.ctrlKey && item && Array.isArray(item.containsItems)) {
                ...
                if (statsContent.classList.contains("sticky-stats")) {
                    ...
                    if (
                        Number(statsContent.getAttribute("data-item-index")) !=
                        itemIndex
                    )
    ...
    if (touchDevice)
        [].forEach.call(
            document.querySelectorAll(".wardrobe-set"),
            bindWardrobeEvents
        );

    [].forEach.call(
        document.querySelectorAll(".rich-item .piece-hover-area"),
        bindLoreEvents
    );
    ...
    [].forEach.call(
        document.querySelectorAll(".enable-api"),
        function (element) {
            element.addEventListener("click", function (e) {
                e.preventDefault();
                dimmer.classList.add("show-dimmer");
    ...
    enableApiPlayer.addEventListener("click", function () {
        if (enableApiPlayer.paused) enableApiPlayer.play();
        else enableApiPlayer.pause();
    });
    ...
    [].forEach.call(
        document.querySelectorAll(".close-lore"),
        function (element) {
    ...
    [].forEach.call(document.querySelectorAll(".copy-text"), function (e) {
        ...
        element.addEventListener("click", function () {
            navigator.clipboard
                .writeText(element.getAttribute("data-copy-text"))
                .then(
                    function () {
    ...
    [].forEach.call(document.querySelectorAll(".add-favorite"), function (e) {
        ...
        element.addEventListener("click", function () {
            if (
                ...
                "0c0b857f415943248f772164bf76795c"
            ) {
    ...
    if (revealSocials) {
        revealSocials.addEventListener("click", function () {
            if (socialsShown) {
                socialsShown = false;
                document
                    .querySelector("#additional_socials")
                    .classList.remove("socials-shown");
                document
                    .querySelector("#reveal_socials")
                    .classList.remove("socials-shown");
            } else {
                socialsShown = true;
                document
                    .querySelector("#additional_socials")
                    .classList.add("socials-shown");
                document
                    .querySelector("#reveal_socials")
                    .classList.add("socials-shown");
            }
    ...
    document.addEventListener("scroll", function () {
        if (!updateTabLock) updateTab = true;
    });
    ...
    [].forEach.call(document.querySelectorAll(".xp-skill"), function (element) {
        ...
        if (skillProgressText === null) return;

        let originalText = skillProgressText.innerHTML;

        element.addEventListener("mouseenter", function () {
            skillProgressText.innerHTML = skillProgressText.getAttribute(
                "data-hover-text"
            );
        });
    ...
    [].forEach.call(
        document.querySelectorAll(".kills-deaths-container .show-all.enabled"),
        function (element) {
    ...
    window.addEventListener("keydown", function (e) {
        ...
        if (
            document.querySelector(".rich-item.sticky-stats") != null &&
            e.keyCode == 9
        )
            e.preventDefault();
    });
