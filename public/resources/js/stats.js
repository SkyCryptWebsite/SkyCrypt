...
document.addEventListener("DOMContentLoaded", function () {
      ...
      /*html*/ `
            <button class="additional-player-stat svg-icon">
                <svg viewBox="0 0 24 24">
                    <title>share</title>
                    <path fill="white" d="${
                      navigator.platform.match(/(Mac|iPhone|iPod|iPad)/i) ? iosShareIcon : androidShareIcon
                    }" />
                </svg>
            </button>
        `
  ...
  function renderInventory(inventory, type) {
    ...
    inventory.forEach(function (item, index) {
      ...
      if (item.id) {
        ...
        if (isEnchanted(item)) inventoryItemIcon.classList.add("is-enchanted");
        ...
        if (type === "backpack") inventoryItem.setAttribute("data-backpack-item-index", index);
        else inventoryItem.setAttribute("data-item-index", item.item_index);

        inventoryItem.appendChild(inventoryItemIcon);
        inventoryItem.appendChild(pieceHoverArea);

        if (item.Count != 1) inventoryItem.appendChild(inventoryItemCount);
  ...
  function showBackpack(item) {
    ...
    if (activeInventory) activeInventory.classList.remove("active-inventory");
    ...
    if (Array.isArray(item.containsItems)) {
      ...
      item.containsItems.forEach((backpackItem, index) => {
        ...
        if (backpackItem.id) {
          ...
          if (backpackItem.Count > 1) inventoryItem.appendChild(inventoryItemCount);
  ...
  function bindWardrobeEvents(element) {
    element.addEventListener("click", function (e) {
      ...
      if (currentWardrobe) currentWardrobe.classList.remove("wardrobe-opened");
    ...
    element.addEventListener("click", function (e) {
      ...
      if (e.ctrlKey && item && Array.isArray(item.containsItems)) {
        ...
        if (statsContent.classList.contains("sticky-stats")) {
          ...
          if (Number(statsContent.getAttribute("data-item-index")) != itemIndex) fillLore(element.parentNode);
        }
      }
    });
  }

  if (touchDevice) [].forEach.call(document.querySelectorAll(".wardrobe-set"), bindWardrobeEvents);
  ...
  window.addEventListener("keydown", function (e) {
    ...
    if (document.querySelector(".rich-item.sticky-stats") != null && e.key === "Tab") e.preventDefault();
  });
