...
const ZOO_CYCLE = ["ELEPHANT", "GIRAFFE", "BLUE_WHALE", "TIGER", "LION", "MONKEY"];

const getOffset = (month: string, day: number) => {
  return MONTHS.indexOf(month) * MONTH_LENGTH * DAY_MS + (day - 1) * DAY_MS;
};

/*const getZooPet = (time: number) => {
  const iterations = Math.floor((time - ZOO_START) / ZOO_CYCLE_MS);

  return ZOO_CYCLE[iterations % ZOO_CYCLE.length];
};*/

const nth = (n: number) => n + ["st", "nd", "rd"][((((n + 90) % 100) - 10) % 10) - 1] || n + "th";
...
/*const EVENTS = [
  ...
  {
    name: "Fear Mongerer",
    times: [[getOffset("Autumn", 26), getOffset("Late Autumn", 3)]],
  },
  {
    name: "Spooky Festival",
    times: [[getOffset("Autumn", 29), getOffset("Autumn", 31)]],
  },
  {
    times: [[getOffset("Late Winter", 24), getOffset("Late Winter", 26)]],
  },
  {
    name: "Jerry Workshop",
    times: [[getOffset("Late Winter", 1), getOffset("Late Winter", 31)]],
  },
  {
    name: "New Year Celebration",
    times: [[getOffset("Late Winter", 29), getOffset("Late Winter", 31)]],
  },
  {
    name: "Election Booth Opens",
    times: [[getOffset("Late Summer", 27), getOffset("Late Summer", 27)]],
  },
  {
    name: "Election Over",
    times: [[getOffset("Late Spring", 27), getOffset("Late Spring", 27)]],
  },
  {
    name: "National Mining Month",
    years: [91],
    times: [[getOffset("Late Summer", 22), getOffset("Early Autumn", 21)]],
  },
];*/
...
function getTime(time: number) {
  ...
  const currentMinute = Math.floor(((currentDayOffset - currentHour * HOUR_MS) / HOUR_MS) * 60);
...
const time = document.getElementById("time_subtext") as HTMLElement;
const updateTimes = () => {
  ...
  //time.innerHTML = `Early Autumn<br>39th<br>${currentTime}pm<br>☀️`;
};
