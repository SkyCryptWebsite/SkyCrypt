...
module.exports = (app, db) => {
    ...
    try {
      ...
      for (const member in coopMembers) {
        ...
        for (const minion of coopMembers[member].crafted_generators) {
          ...
          if (minionObject.level < minionLevel) minionObject.level = minionLevel;
        }
      }

      if ("html" in req.query) res.send(tableify(minions, { showHeaders: false }));
  ...
  app.all("/api/:player/:profile/weapons", cors(), async (req, res) => {
    try {
        ...
        if (hasPath(weapon, "tag", "ExtraAttributes")) {
          if ("html" in req.query) {
            ...
            if (stats !== undefined) {
              ...
              for (const stat in stats) statsOutput.push(stat + "=" + stats[stat]);
            ...
            if (extra !== undefined) {
              ...
              for (const value in extra) extraOutput.push(value + "=" + extra[value]);
      ...
      if ("html" in req.query) res.send(tableify(output, { showHeaders: false }));
      else res.status(403).send("Old JSON API has been disabled.");
    } catch (e) {
      handleError(e, res);
    }
