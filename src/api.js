...
module.exports = (app, db) => {
    ...
    async function init() {
        ...
        for (const product of bazaarProducts) {
            ...
            if (info.length > 0) productInfo[product.productId] = info[0];
        }
    }
                ...
                const members = (
                    await Promise.all(
                        Object.keys(profile.members).map((a) =>
                            helper.resolveUsernameOrUuid(a, db)
                        )
                    )
    ...
    app.all("/api/:player/:profile/pets", cors(), async (req, res) => {
        try {
            const { profile, uuid } = await lib.getProfile(
                db,
                req.params.player,
                req.params.profile
            );
        ...
        try {
            ...
            for (const member in coopMembers) {
                ...
                for (const minion of coopMembers[member].crafted_generators) {
                    ...
                    if (
                        minions.filter((a) => a.minion == minionName).length ==
                        0
                    )
                        minions.push({
                            minion: minionName,
                    ...
                    let minionObject = minions.filter(
    ...
    app.all("/api/:player/:profile/collections", cors(), async (req, res) => {
        try {
            ...
            for (const collection in collections)
                collections[collection].name = constants.collection_data.filter(
                    (a) => a.skyblockId == collection
                )[0].name;
    ...
    app.all("/api/:player/:profile/skills", cors(), async (req, res) => {
        try {
            const { profile, allProfiles, uuid } = await lib.getProfile(
                db,
                req.params.player,
                req.params.profile
            );
    ...
    app.all("/api/:player/:profile/cakebag", cors(), async (req, res) => {
        try {
            ...
            const allItems = items.armor.concat(
                items.inventory,
                items.talisman_bag,
                items.enderchest
            );

            const cakeBags = allItems.filter(
                (a) =>
                    helper.getPath(a, "tag", "ExtraAttributes", "id") ==
                    "NEW_YEAR_CAKE_BAG"
            );
            ...
            if (cakeBags.length == 0) {
                ...
                for (const item of cakeBag.containsItems)
                    if (
                        ...
                        cakes.push({
                            cake: item.tag.ExtraAttributes.new_years_cake,
                        });
    ...
    app.all("/api/bazaar", cors(), async (req, res) => {
        ...
        try {
            ...
            for await (const product of db.collection("bazaar").find()) {
                ...
                const productName = itemInfo
                    ? itemInfo.name
                    : helper.titleCase(product.productId.replace(/(_+)/g, " "));
