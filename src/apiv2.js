...
module.exports = (app, db) => {
    ...
    const init = new Promise(async (resolve, reject) => {
        ...
        for (const key of keys) {
            ...
            if (
                lb.mappedBy == "uuid" &&
                !lb.key.startsWith("collection_enchanted")
            )
    ...
    app.use("/api/v2/*", async (req, res, next) => {
        ...
        if (req.query.key) {
            ...
            if (doc != null) req.cacheOnly = false;
        }
    ...
    app.all("/api/v2/leaderboard/:lbName", cors(), async (req, res) => {
        ...
        const lbCount = await redisClient.zcount(
            `lb_${lb.key}`,
            "-Infinity",
            "+Infinity"
        );
        ...
        if (req.query.find) {
            const uuid = (
                await helper.resolveUsernameOrUuid(req.query.find, db, true)
            ).uuid;
        ...
        for (let i = 0; i < results.length; i += 2) {
            const lbPosition = {
                ...
                uuid: results[i],
                username: (
                    await helper.resolveUsernameOrUuid(results[i], db, true)
                ).display_name,
            };
            ...
            for (const singleProfile of allProfiles) {
                ...
                output.profiles[singleProfile.profile_id] = {
                    ...
                    current:
                        Math.max(
                            ...allProfiles.map(
                                (a) => a.members[profile.uuid].last_save
                            )
                        ) == userProfile.last_save,
        ...
        try {
            ...
            for (const singleProfile of allProfiles) {
                ...
                const items = await lib.getItems(
                    singleProfile.members[profile.uuid],
                    req.query.pack
                );
                const data = await lib.getStats(
                    db,
    ...
    app.all("/api/v2/dungeons/:player/:profile", cors(), async (req, res) => {
        try {
            ...
            for (const singleProfile of allProfiles) {
                ...
                const dungeonData = await lib.getDungeons(
                    userProfile,
                    hypixelProfile
                );
