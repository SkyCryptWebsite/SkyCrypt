...
async function main() {
    ...
    async function getExtra(page = null, favorite = false) {
        ...
        const patreonEntry = await db
            .collection("donations")
            .findOne({ type: "patreon" });
    ...
    app.all("/stats/:player/:profile?", async (req, res, next) => {
        let paramPlayer = req.params.player
            .toLowerCase()
            .replace(/[^a-z\d\-\_:]/g, "");
        let paramProfile = req.params.profile
            ? req.params.profile.toLowerCase()
            : null;

        const cacheOnly = req.query.cache === "true";

        const playerUsername =
            paramPlayer.length == 32
                ? await helper.resolveUsernameOrUuid(paramPlayer, db)
                      .display_name
                : paramPlayer;

        try {
            const { profile, allProfiles } = await lib.getProfile(
                db,
                paramPlayer,
                paramProfile,
                { updateArea: true, cacheOnly }
            );

            const items = await lib.getItems(
                profile.members[profile.uuid],
                true,
                req.cookies.pack
            );
            const calculated = await lib.getStats(
                db,
                profile,
                allProfiles,
                items
    ...
    app.all("/cape/:username", cors(), async (req, res) => {
        ...
        try {
            ...
            if (Date.now() - stats.mtime > 10 * 1000) {
                const optifineCape = await axios.head(
                    `https://optifine.net/capes/${username}.png`
                );
                const lastUpdated = moment(
                    optifineCape.headers["last-modified"]
                );
    ...
    app.all("/head/:uuid", cors(), async (req, res) => {
        ...
        try {
            ...
            file = await renderer.renderHead(
    ...
    app.all("/leather/:type/:color", cors(), async (req, res) => {
        ...
        if (
            !["boots", "leggings", "chestplate", "helmet"].includes(
                req.params.type
            )
        )
        ...
        if (color.length < 3) throw new Error("invalid color");
...
`
    ...
    app.all("/random/stats", async (req, res, next) => {
        ...
        res.redirect(
            `/stats/20934ef9488c465180a78f861586b4cf/bf7c14fb018946899d944d56e65222d2`
        );
    });
    ...
    app.listen(port, () =>
        console.log(`SkyBlock Stats running on http://localhost:${port}`)
    );
}
