...
const raceFormat = (x) => {
    ...
    if (x < 1000) raceDuration = "0." + raceDuration;
...
const skillFormat = (xp) => {
    ...
    return `Level ${
        levelObj.level
    } + ${levelObj.xpCurrent.toLocaleString()} XP`;
};
...
module.exports = {
    leaderboard: (name) => {
        ...
        if (lbName.startsWith("collection_")) {
            const collectionName = lbName
                .split("_")
                .slice(1)
                .join("_")
                .toUpperCase();
            const collectionData = collections.collection_data.filter(
                (a) => a.skyblockId == collectionName
            );
