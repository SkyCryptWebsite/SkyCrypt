...
const RESOURCE_PACK_FOLDER = path.resolve(
    __dirname,
    "..",
    "public",
    "resourcepacks"
...
async function getFiles(dir, fileList) {
    ...
    for (const file of files) {
        ...
        else fileList.push(path.resolve(dir, file));
    }
...
async function init() {
    ...
    for (let pack of resourcePacks) {
        ...
        for (const file of pack.files) {
            if (path.extname(file) != ".properties") continue;
            ...
            if (properties.type != "item") continue;
            ...
            let textureFile =
                "texture" in properties
                    ...
                    : path.resolve(
                          path.dirname(file),
                          path.basename(file, ".properties")
                      );

            if ("texture.bow_standby" in properties)
                textureFile = path.resolve(
                    path.dirname(file),
                    properties["texture.bow_standby"]
                );
            ...
            if ("model" in properties) {
                ...
                try {
                    const model = RJSON.parse(
                        await fs.readFile(modelFile, "utf8")
                    );

                    if (model.parent == "builtin/generated") {
                        const layers = Object.keys(model.textures).sort(
                            (a, b) => a - b
                        );
            ...
            if (
                Object.keys(properties).filter((a) =>
                    a.includes("texture.leather_")
                ).length == 2
            ) {
                try {
                    const leatherProperties = Object.keys(
                        properties
                    ).filter((a) => a.includes("texture.leather_"));

                    let leatherBase =
                        properties[
                            leatherProperties.filter(
                                (a) => !a.includes("_overlay")
                            )[0]
                        ];
                    let leatherOverlay =
                        properties[
                            leatherProperties.filter((a) =>
                                a.includes("_overlay")
                            )[0]
                        ];

                    if (!leatherBase.endsWith(".png")) leatherBase += ".png";
                        ...
                        if (leatherMetadata.width != NORMALIZED_SIZE)
                            await fs.writeFile(
                                ...
                                await leatherImage
                                    .resize(
                                        NORMALIZED_SIZE,
                                        leatherMetadata.height *
                                            (NORMALIZED_SIZE /
                                                leatherMetadata.width),
                                        {
                ...
                Object.keys(properties).filter(
                    (a) =>
                        a.includes("texture.leather_") && a.includes("_overlay")
                ).length == 1
            ) {
                    ...
                    await textureImage
                        .resize(
                            ...
                            textureMetadata.height *
                                (NORMALIZED_SIZE / textureMetadata.width),
                            {
                                kernel: sharp.kernel.nearest,
                            }
                        )
            ...
            for (let property in properties) {
                ...
                if (property == "items" || property == "matchItems") {
                    let item = mcData.findItemOrBlockByName(
                        properties[property].replace("minecraft:", "")
                    );

                    if (item) texture.id = item.id;
                }

                if (property == "damage")
                    texture.damage = parseInt(properties[property]);

                if (!property.startsWith("nbt.")) continue;
            ...
            if (
                ...
                for (const [index, frame] of animation.frames.entries()) {
                    ...
                    animation.frames[index].time =
                        (animation.frames[index].time / 20) * 1000;
                ...
                if ("frames" in animation) {
                    ...
                    if (animation.interpolate) {
                        ...
                        let frameCountInterpolated =
                            totalLength / frameTimeInterpolated;

                        for (let i = 0; i < frameCountInterpolated; i++) {
                            let frameCur, frameNext;
                            let currentTime =
                                (i / frameCountInterpolated) * totalLength;

                            for (const [
                                index,
                                frame,
                            ] of animation.frames.entries()) {
                                if (
                                    frame.totalTime + frame.time >
                                    currentTime
                                ) {
                                    frameCur = frame;
                            ...
                            const opacity =
                                (currentTime - frameCur.totalTime) /
                                frameCur.time;
                            ...
                            ctx.drawImage(
                                getFrame(image, frameNext.index),
                                0,
                                0
                            );
                ...
                if (pngFrames.length > 0) {
                    ...
                    await execFile(apng2gif, [
                        textureFile,
                        textureFile.replace(".png", ".gif"),
                    ]);
                }
            }
...
module.exports = {
    ...
    getTexture: async (item, ignoreId = false, packIds) => {
        if (!module.exports.ready) await readyPromise;
        ...
        if (packIds.length > 0)
            _resourcePacks = _resourcePacks.filter((a) =>
                packIds.includes(a.config.id)
            );

        _resourcePacks = _resourcePacks.sort(
            (a, b) => packIds.indexOf(a) - packIds.indexOf(b)
        );

        for (const pack of _resourcePacks) {
            if ("weight" in outputTexture) outputTexture.weight = -9999;

            for (const texture of pack.textures) {
                if (ignoreId === false && texture.id != item.id) continue;

                if (
                    ignoreId === false &&
                    "damage" in texture &&
                    texture.damage != item.Damage
                )
                    continue;
                ...
                for (const match of texture.match) {
                    ...
                    let matchValues = helper.getPath(
                        item,
                        "tag",
                        ...value.split(".")
                    );
                ...
                if (matches == texture.match.length) {
                    if (texture.weight < outputTexture.weight) continue;

                    if (
                        texture.weight == outputTexture.weight &&
                        texture.file < outputTexture.file
                    )
                        continue;
        ...
        if (!("path" in outputTexture)) return null;
