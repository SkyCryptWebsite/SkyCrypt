...
module.exports = {
    ...
    setPath: (obj, value, ...keys) => {
        ...
        for (i = 0; i < keys.length - 1; i++) {
            if (!loc.hasOwnProperty(keys[i])) loc[keys[i]] = {};
    ...
    resolveUsernameOrUuid: async (uuid, db, cacheOnly = false) => {
        ...
        if (isUuid) {
            user = await db.collection("usernames").findOne({ uuid });
        } else {
        ...
        if (
            cacheOnly === false &&
            (user === null || +new Date() - user.date > 7200 * 1000)
        ) {
            let profileRequest = axios(
                `https://api.ashcon.app/mojang/v1/user/${uuid}`,
                { timeout: 5000 }
            );
                ...
                .then(async (response) => {
                    try {
                        ...
                        if (module.exports.hasPath(data.textures, "skin")) {
                            ...
                            skin_data.model = data.textures.slim
                                ? "slim"
                                : "regular";
                        }
                        ...
                        for await (const doc of playerObjects) {
                            ...
                            if (
                                ...
                                module.exports
                                    .resolveUsernameOrUuid(doc.uuid, db)
                                    .catch(console.error);
                            }
                        }
                    } catch (e) {
            ...
            if (!user) {
                try {
                    ...
                    if (isUuid) {
                        ...
                        else throw "Failed resolving username.";
                    }
                }
            }
        }
        ...
        else return { uuid, display_name: uuid, skin_data };
    },
    ...
    getGuild: async (uuid, db, cacheOnly = false) => {
        ...
        if (cacheOnly && guildMember === null) return null;
        ...
        if (
            ...
            (guildMember !== null &&
                ...
                (guildObject === null ||
                    Date.now() - guildMember.last_updated < 7200 * 1000))
        ) {
            if (guildMember.gid !== null) {
                ...
                if (guildObject === null) return null;

                guildObject.level = module.exports.getGuildLevel(
                    guildObject.exp
                );
                guildObject.gmUser = await module.exports.resolveUsernameOrUuid(
                    guildObject.gm,
                    db,
                    cacheOnly
                );
                ...
                try {
                    ...
                    if (guild && guild !== null) {
                        for (const member of guild.members)
                            if (
                                ["guild master", "guildmaster"].includes(
                        ...
                        for (const member of guild.members) {
                            ...
                            await db.collection("guildMembers").updateOne(
                                { uuid: member.uuid },
                                {
                                    $set: {
                                        gid: guild._id,
                        ...
                        guildObject.value.level = module.exports.getGuildLevel(
                            guildObject.value.exp
                        );
                        guildObject.value.gmUser = await module.exports.resolveUsernameOrUuid(
                            guildObject.value.gm,
                            db
                        );
                        guildObject.value.rank = guild.members.filter(
                            (a) => a.uuid == uuid
                        )[0].rank;

                        return guildObject.value;
                    } else {
                        await db.collection("guildMembers").findOneAndUpdate(
                            { uuid },
                            {
                                $set: {
                                    gid: null,
    ...
    getGuildLevel: (xp) => {
        ...
        for (let i = 0; ; i++) {
            const xpNeeded =
                constants.guild_xp[Math.min(constants.guild_xp.length - 1, i)];
    ...
    getPrice: (orderSummary) => {
        orderSummary = orderSummary.slice(
            0,
            Math.ceil(orderSummary.length / 2)
        );

        const orders = [];

        const totalVolume = orderSummary
            .map((a) => a.amount)
            .reduce((a, b) => a + b, 0);
        ...
        for (const order of orderSummary) {
            ...
            if (volume >= volumeTop2) break;
        }

        const totalWeight = orders.reduce((sum, value) => sum + value[1], 0);

        return orders.reduce(
            (mean, value) => mean + (value[0] * value[1]) / totalWeight,
            0
        );
    },
    ...
    formatNumber: (number, floor, rounding = 10) => {
        if (number < 1000) return Math.floor(number);
            ...
            if (floor) return Math.floor(number / 1000) + "K";
            else return Math.ceil(number / 1000) + "K";
        else if (number < 1000000000)
            if (floor)
                return (
                    (
                        Math.floor((number / 1000 / 1000) * rounding) / rounding
                    ).toFixed(rounding.toString().length - 1) + "M"
                );
            else
                return (
                    (
                        Math.ceil((number / 1000 / 1000) * rounding) / rounding
                    ).toFixed(rounding.toString().length - 1) + "M"
                );
        else if (floor)
            return (
    ...
    parseRank: (player) => {
        ...
        if (
            module.exports.hasPath(player, "monthlyPackageRank") &&
        ...
        if (module.exports.hasPath(player, "prefix"))
            rankName = module.exports
                .getRawLore(player.prefix)
                .replace(/\[|\]/g, "");
        ...
        if (rankName == "PIG+++") output.plusColor = "b";
    ...
    renderRank: (rank) => {
        ...
        if (rankText === null) return output;

        rankColor =
            constants.minecraft_formatting[rankColor].niceColor ||
            constants.minecraft_formatting[rankColor].color;

        output = `<div class="rank-tag ${
            plusText ? "rank-plus" : ""
        }"><div class="rank-name" style="background-color: ${rankColor}">${rankText}</div>`;

        if (plusText) {
            plusColor =
                constants.minecraft_formatting[plusColor].niceColor ||
    ...
    updateRank: async (uuid, db) => {
        ...
        await db
            ...
            .updateOne({ uuid }, { $set: rank }, { upsert: true });
    ...
    fetchMembers: async (profileId, db, returnUuid = false) => {
        ...
        if (members.length == 0) {
            ...
            for (const member in profileResponse.data.profile.members)
                memberPromises.push(
                    module.exports.resolveUsernameOrUuid(member, db)
                );
            ...
            for (const profileMember of profileMembers) {
                await db.collection("members").replaceOne(
                    { profile_id: profileId, uuid: profileMember.uuid },
                    {
                        ...
                        uuid: profileMember.uuid,
                        username: profileMember.display_name,
                    },
                    { upsert: true }
                );
            }

            if (returnUuid) output = profileMembers;
            else output = profileMembers.map((a) => a.display_name);
        } else {
            if (returnUuid)
                output = members.map((a) => {
                    return { uuid: a.uuid, display_name: a.username };
                });
            else output = members.map((a) => a.username);
        }
