...
module.exports = {
  ...
  getGuild: async (uuid, db, cacheOnly = false) => {
    const guildMember = await db.collection("guildMembers").findOne({ uuid: uuid });
    ...
    if (guildMember !== null && guildMember.gid !== null) {
      guildObject = await db.collection("guilds").findOne({ gid: guildMember.gid });
    }
    ...
    if (
      ...
      if (guildMember.gid !== null) {
        const guildObject = await db.collection("guilds").findOne({ gid: guildMember.gid });
  ...
  updateRank: async (uuid, db) => {
    ...
    await db.collection("hypixelPlayers").updateOne({ uuid }, { $set: rank }, { upsert: true });
