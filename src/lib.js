...
function calculateFarmingWeight(userProfile) {
  const output = {
    weight: 0,
    crops: {},
    bonuses: {
  ...
  if (userProfile?.collections) {
    ...
    let mushroom_collection = userProfile?.collections?.MUSHROOM_COLLECTION.amount;

    let total = output.weight + mushroom_collection / 168_925.53;
    ...
    let mushroom_weight =
      double_break_ratio * (mushroom_collection / (2 * 168_925.53)) + normal_ratio * (mushroom_collection / 168_925.53);
  ...
  if (userProfile?.levels?.farming) {
    if (userProfile.levels.farming.level >= 50) {
      output.bonuses.level.level = 50;
      output.bonuses.level.weight += 100;
      bonus += 100;
    }
    if (userProfile.levels.farming.level >= 60) {
  ...
  if (userProfile?.farming) {
    let double_drops = userProfile.farming?.perks?.double_drops ?? 0;
    ...
    let gold_medals = userProfile.farming?.total_badges?.gold ?? 0;
  ...
  if (userProfile?.minions) {
    ...
    userProfile.minions.forEach((minion) => {
