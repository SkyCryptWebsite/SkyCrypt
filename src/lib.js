...
function getMinMax(profiles, min, ...path) {
    ...
    const compareValues = profiles
        .map((a) => helper.getPath(a, ...path))
        .filter((a) => !isNaN(a));

    if (compareValues.length == 0) return output;

    if (min) output = Math.min(...compareValues);
    else output = Math.max(...compareValues);
...
function getAllKeys(profiles, ...path) {
    return _.uniq(
        [].concat(...profiles.map((a) => _.keys(helper.getPath(a, ...path))))
    );
}
...
function getXpByLevel(level, extra = {}) {
    ...
    if (levelCap > maxLevel) maxLevel = levelCap;
    ...
    for (let x = 1; x <= level; x++) xpTotal += xp_table[x];

    output.xp = xpTotal;

    if (level >= maxLevel) output.progress = 1;
    else output.xpForNext = xp_table[level + 1];
...
function getLevelByXp(xp, extra = {}) {
    ...
    if (extra.cap) levelCap = extra.cap;
    ...
    if (level < levelCap) xpForNext = Math.ceil(xp_table[level + 1]);
...
function getBonusStat(level, skill, max, incremention) {
    ...
    for (let x = steps[0]; x <= max; x += incremention) {
        ...
        for (let skill in skill_bonus) bonus[skill] += skill_bonus[skill];
    }
...
async function getItems(
    ...
    for (const [index, item] of items.entries()) {
        if (
            ...
            if (!Array.isArray(backpackData)) continue;
    ...
    for (const item of items) {
        ...
        if (helper.hasPath(item, "display_name"))
            if (item.display_name == "Water Bottle") item.Damage = 17;
        ...
        if (
            ...
            try {
                const json = JSON.parse(
                    Buffer.from(
                        item.tag.SkullOwner.Properties.textures[0].Value,
                        "base64"
                    ).toString()
                );
        ...
        if (customTextures) {
            const customTexture = await customResources.getTexture(
                item,
                false,
                packs
            );
            ...
            if (customTexture) {
                ...
                item.texture_pack.base_path =
                    ...
                    path.relative(
                        ...
                        customTexture.pack.basePath
                    );
            }
        }

        const enchantments =
            helper.getPath(item, "tag", "ExtraAttributes", "enchantments") ||
            {};
        ...
        let lore =
            lore_raw != null
                ? lore_raw.map((a) => (a = helper.getRawLore(a)))
                : [];
        ...
        if (itemLore.length > 0) {
            if (
                ...
                if (rarity_upgrades > 0) itemLore.push("§8(Recombobulated)");
            }
            ...
            if (
                ...
                if (expertise_kills > 0 && lore_raw) {
                    itemLore.push(
                        "",
                        `§7Expertise Kills: §c${expertise_kills}`
                    );
                    if (expertise_kills >= 15000) itemLore.push(`§8MAXED OUT!`);
                    else {
                        let toNextLevel = 0;
            ...
            if (helper.hasPath(item, "tag", "ExtraAttributes", "timestamp")) {
                ...
                else obtainmentDate = moment(timestamp, "D/M/YY HH:mm");
                ...
                itemLore.push(
                    "",
                    `§7Obtained: §c${obtainmentDate.format("D MMM YYYY")}`
                );
            }

            if (helper.hasPath(item, "tag", "ExtraAttributes", "spawnedFor")) {
                if (
                    !helper.hasPath(item, "tag", "ExtraAttributes", "timestamp")
                )
                    itemLore.push("");

                const spawnedFor = item.tag.ExtraAttributes.spawnedFor.replace(
                    /\-/g,
                    ""
                );
                const spawnedForUser = await helper.resolveUsernameOrUuid(
                    spawnedFor,
                    db,
                    cacheOnly
                );
                ...
                itemLore.push(
                    `§7By: §c<a href="/stats/${spawnedFor}">${spawnedForUser.display_name}</a>`
                );
            }
            ...
            if (helper.hasPath(item, "tag", "ExtraAttributes", "winning_bid")) {
                ...
                itemLore.push(
                    `§7Price Paid at Dark Auction: §b${price.toLocaleString()} coins`
                );
            }
        }
        ...
        if (lore.length > 0) {
            ...
            if (rarity_type.startsWith("a "))
                rarity_type = rarity_type
                    .substring(2)
                    .substring(0, rarity_type.length - 4);
            ...
            if (rarity_type.length > 1) item_type = rarity_type[1].trim();
            ...
            if (loreRarity != colorRarity) item.localized = true;

            if (item_type) item.type = item_type.toLowerCase();

            if (item.type == "hatccessory") item.type = "accessory";
            ...
            if (getId(item) == "SPEED_TALISMAN") item.stats.speed = 1;

            if (getId(item) == "SPEED_RING") item.stats.speed = 3;

            if (getId(item) == "SPEED_ARTIFACT") item.stats.speed = 5;
        }
        ...
        if (
            !helper.hasPath(item, "display_name") &&
            helper.hasPath(item, "id")
        ) {
    ...
    for (let item of items) {
        if (item.inBackpack) {
            items[item.backpackIndex].containsItems.push(
                Object.assign({}, item)
            );
        }
    }
...
module.exports = {
    ...
    getMinions: (coopMembers) => {
        ...
        for (const member in coopMembers) {
            if (!("crafted_generators" in coopMembers[member])) continue;
        ...
        for (const generator of craftedGenerators) {
            ...
            if (minion.length == 0)
                minions.push(
                    Object.assign(
                        { id: minionName, maxLevel: 0, levels: [minionLevel] },
                        constants.minions[minionName]
                    )
                );
            else minion[0].levels.push(minionLevel);
        }
        ...
        for (const minion in constants.minions)
            if (minions.filter((a) => a.id == minion).length == 0)
                minions.push(
                    Object.assign(
                        { id: minion, levels: [], maxLevel: 0 },
                        constants.minions[minion]
                    )
                );
        ...
        for (const minion of minions) {
            ...
            minion.maxLevel =
                minion.levels.length > 0 ? Math.max(...minion.levels) : 0;
    ...
    getMinionSlots: (minions) => {
        ...
        for (const minion of minions) uniqueMinions += minion.levels.length;

        const output = { currentSlots: 5, toNext: 5 };

        const uniquesRequired = Object.keys(constants.minion_slots).sort(
            (a, b) => parseInt(a) - parseInt(b)
        );

        for (const [index, uniques] of uniquesRequired.entries()) {
            if (parseInt(uniques) <= uniqueMinions) continue;

            output.currentSlots =
                constants.minion_slots[uniquesRequired[index - 1]];
    ...
    getItems: async (
        ...
        let wardrobe_inventory =
            "wardrobe_contents" in profile
                ? await getItems(
                      profile.wardrobe_contents.data,
                      customTextures,
                      packs,
                      cacheOnly
                  )
        ...
        let enderchest =
            "ender_chest_contents" in profile
                ? await getItems(
                      profile.ender_chest_contents.data,
                      customTextures,
                      packs,
                      cacheOnly
                  )
        ...
        let candy_bag =
            "candy_inventory_contents" in profile
                ? await getItems(
                      profile.candy_inventory_contents.data,
                      customTextures,
                      packs,
                      cacheOnly
                  )
        ...
        enderchest = enderchest.map((a) =>
            Object.assign({ isInactive: true }, a)
        );
        ...
        for (const talisman of armor.filter((a) => a.type == "accessory")) {
            ...
            if (
                talismans.filter((a) => !a.isInactive && getId(a) == id)
                    .length > 0
            )
        ...
        for (const talisman of inventory.filter(
            (a) => a.type == "accessory" || a.type == "dungeon accessory"
        )) {
            const id = getId(talisman);
        ...
        for (const talisman of talismans.concat(armor)) {
            ...
            if (id.startsWith("CAMPFIRE_TALISMAN_")) {
                ...
                const maxTier = Math.max(
                    ...talismans
                        .filter((a) =>
                            getId(a).startsWith("CAMPFIRE_TALISMAN_")
                        )
                        .map((a) => getId(a).split("_").pop())
                );
            ...
            if (id.startsWith("WEDDING_RING_")) {
                ...
                const maxTier = Math.max(
                    ...talismans
                        .filter((a) => getId(a).startsWith("WEDDING_RING_"))
                        .map((a) => getId(a).split("_").pop())
                );
            ...
            if (id in constants.talisman_upgrades) {
                ...
                if (
                    talismans.filter(
                        (a) =>
                            !a.isInactive && talismanUpgrades.includes(getId(a))
                    ).length > 0
                )
                    talisman.isInactive = true;

                if (
                    talismans.filter((a) => talismanUpgrades.includes(getId(a)))
                        .length > 0
                )
                    talisman.isUnique = false;
            }
            ...
            if (id in constants.talisman_duplicates) {
                ...
                if (
                    talismans.filter((a) =>
                        talismanDuplicates.includes(getId(a))
                    ).length > 0
                )
                    talisman.isUnique = false;
            }
        ...
        for (const talisman of talismans) {
            ...
            if (
                helper.hasPath(talisman, "tag", "ExtraAttributes", "modifier")
            ) {
                talisman.base_name = talisman.display_name
                    .split(" ")
                    .slice(1)
        ...
        output.weapons = all_items.filter(
            (a) =>
                a.type != null &&
                (a.type.endsWith("sword") || a.type.endsWith("bow"))
        );
        output.rods = all_items.filter(
            (a) => a.type != null && a.type.endsWith("fishing rod")
        );
        ...
        if (personal_vault.length == 0) output.no_personal_vault = true;
        ...
        let swordsInventory = swords.filter(
            (a) => a.backpackIndex === undefined
        );
        let bowsInventory = bows.filter((a) => a.backpackIndex === undefined);
        let rodsInventory = output.rods.filter(
            (a) => a.backpackIndex === undefined
        );
        ...
        if (armor.filter((a) => Object.keys(a).length > 2).length == 4) {
            ...
            const isPerfectSet =
                armor.filter((a) => getId(a).startsWith("PERFECT_")).length ==
                4;

            const isSpongeSet =
                armor.filter((a) => getId(a).startsWith("SPONGE_")).length == 4;

            if (
                armor.filter(
                    (a) =>
                        a.armor_name.split(" ")[0] ==
                        armor[0].armor_name.split(" ")[0]
                ).length == 4 ||
                isMonsterSet
            ) {
                ...
                if (
                    !output_name.endsWith("Armor") &&
                    !output_name.startsWith("Armor")
                )
                    output_name += " Armor";
                ...
                if (isSpongeSet) output.armor_set = "Sponge Armor";
    ...
    getLevels: async (userProfile, hypixelProfile) => {
        ...
        if (
            ...
            skillLevels = {
                ...
                farming: getLevelByXp(
                    userProfile.experience_skill_farming || 0,
                    {
                        skill: "farming",
                        cap:
                            hypixelProfile.achievements.skyblock_harvester || 0,
                    }
                ),
                ...
                foraging: getLevelByXp(
                    userProfile.experience_skill_foraging || 0,
                    { skill: "foraging" }
                ),
                fishing: getLevelByXp(
                    userProfile.experience_skill_fishing || 0,
                    { skill: "fishing" }
                ),
                enchanting: getLevelByXp(
                    userProfile.experience_skill_enchanting || 0,
                    { skill: "enchanting" }
                ),
                alchemy: getLevelByXp(
                    userProfile.experience_skill_alchemy || 0,
                    { skill: "alchemy" }
                ),
                carpentry: getLevelByXp(
                    userProfile.experience_skill_carpentry || 0,
                    { skill: "carpentry" }
                ),
                runecrafting: getLevelByXp(
                    userProfile.experience_skill_runecrafting || 0,
                    { skill: "runecrafting", type: "runecrafting" }
                ),
            };
            ...
            for (let skill in skillLevels) {
                if (skill != "runecrafting" && skill != "carpentry") {
                    average_level +=
                        skillLevels[skill].level + skillLevels[skill].progress;
            ...
            output.average_level =
                average_level / (Object.keys(skillLevels).length - 2);
            output.average_level_no_progress =
                average_level_no_progress /
                (Object.keys(skillLevels).length - 2);
            ...
            skillLevels = {
                ...
                enchanting:
                    hypixelProfile.achievements.skyblock_augmentation || 0,
            ...
            for (const skill in skillLevels) {
                ...
                if (skillLevels[skill] < 0) continue;
        ...
        output.average_level_rank = await redisClient.zcount([
            `lb_average_level`,
            output.average_level,
            "+inf",
        ]);
    ...
    getStats: async (db, profile, allProfiles, items, cacheOnly = false) => {
        ...
        const hypixelProfile = await helper.getRank(
            profile.uuid,
            db,
            cacheOnly
        );
        ...
        if (userProfile.fairy_exchanges > 0) {
            ...
            for (let stat in fairyBonus) output.stats[stat] += fairyBonus[stat];
        }
        ...
        output.fairy_souls = {
            ...
            total: MAX_SOULS,
            progress: Math.min(
                userProfile.fairy_souls_collected / MAX_SOULS,
                1
            ),
        };
        ...
        const {
            ...
            total_skill_xp,
            average_level_rank,
        } = await module.exports.getLevels(userProfile, hypixelProfile);
        ...
        for (let skill in levels) {
            if (levels[skill].level == 0) continue;
        ...
        if ("slayer_bosses" in userProfile) {
            ...
            if (helper.hasPath(userProfile, "slayer_bosses")) {
                for (const slayerName in userProfile.slayer_bosses) {
                    ...
                    if (!helper.hasPath(slayer, "claimed_levels")) continue;

                    slayers[slayerName].level = getSlayerLevel(
                        slayer,
                        slayerName
                    );
                    ...
                    for (const property in slayer) {
                        ...
                        if (property.startsWith("boss_kills_tier_")) {
                            const tier =
                                parseInt(
                                    property.replace("boss_kills_tier_", "")
                                ) + 1;

                            slayers[slayerName].kills[tier] = slayer[property];

                            output.slayer_coins_spent[slayerName] =
                                (output.slayer_coins_spent[slayerName] || 0) +
                                slayer[property] * constants.slayer_cost[tier];
                        }
                    }
                }
            ...
            for (const slayer in slayers) {
                ...
                const slayerBonus = getBonusStat(
                    slayers[slayer].level.currentLevel,
                    `${slayer}_slayer`,
                    9,
                    1
                );
        ...
        if (!items.no_inventory) {
            output.missingTalismans = await module.exports.getMissingTalismans(
                items.talisman_ids
            );
        ...
        const petScoreRequired = Object.keys(constants.pet_rewards).sort(
        ...
        for (const [index, score] of petScoreRequired.entries()) {
            if (parseInt(score) > output.petScore) continue;

            output.pet_score_bonus = Object.assign(
                {},
                constants.pet_rewards[score]
            );
        ...
        for (const pet of output.pets) {
            ...
            for (const stat in pet.stats)
                output.pet_bonus[stat] =
                    (output.pet_bonus[stat] || 0) + pet.stats[stat];
        }

        // Apply all harp bonuses when Melody's Hair has been acquired
        if (
            items.talismans.filter((a) => getId(a) == "MELODY_HAIR").length == 1
        )
        ...
        if (
            items.armor.filter((a) => getId(a).startsWith("LAPIS_ARMOR_"))
                .length == 4
        )
            items.armor[0].stats.health =
                (items.armor[0].stats.health || 0) + 60;
        ...
        if (
            ...
            items.armor.filter((a) => getId(a).startsWith("EMERALD_ARMOR_"))
                .length == 4
        ) {
            let emerald_bonus = Math.min(
                350,
                Math.floor(userProfile.collection.EMERALD / 3000)
            );
        ...
        if (
            items.armor.filter((a) => getId(a).startsWith("FAIRY_")).length == 4
        )
            items.armor[0].stats.speed += 10;

        // Apply Speedster Armor full set bonus of +20 Speed
        if (
            items.armor.filter((a) => getId(a).startsWith("SPEEDSTER_"))
                .length == 4
        )
        ...
        if (
            items.armor.filter((a) => getId(a).startsWith("YOUNG_DRAGON_"))
                .length == 4
        )
        ...
        for (const item of items.armor) {
            ...
            for (let stat in item.stats) output.stats[stat] += item.stats[stat];
        }
        ...
        if (
            items.armor.filter((a) => getId(a).startsWith("MASTIFF_")).length ==
            4
        ) {
        ...
        if (
            items.talismans.filter(
                (a) =>
                    !a.isInactive &&
                    ["DAY_CRYSTAL", "NIGHT_CRYSTAL"].includes(getId(a))
            ).length == 2
        ) {
            output.stats.defense += 5;
            output.stats.strength += 5;

            const dayCrystal = items.talismans.filter(
                (a) => getId(a) == "DAY_CRYSTAL"
            )[0];
        ...
        if (
            items.armor.filter((a) => getId(a) == "OBSIDIAN_CHESTPLATE")
                .length == 1
        ) {
            let obsidian = 0;

            for (let item of items.inventory) {
                if (item.id == 49) obsidian += item.Count;
            }
        ...
        if (
            items.armor.filter((a) => getId(a).startsWith("CHEAP_TUXEDO_"))
                .length == 3
        )
            output.stats["health"] = 75;

        if (
            items.armor.filter((a) => getId(a).startsWith("FANCY_TUXEDO_"))
                .length == 3
        )
            output.stats["health"] = 150;

        if (
            items.armor.filter((a) => getId(a).startsWith("ELEGANT_TUXEDO_"))
                .length == 3
        )
        ...
        for (const item of items.weapons.concat(items.rods)) {
            ...
            if (
                item.stats.crit_damage > 0 &&
                items.armor.filter((a) => getId(a).startsWith("MASTIFF_"))
                    .length == 4
            )
            ...
            if (
                items.armor.filter((a) =>
                    getId(a).startsWith("SUPERIOR_DRAGON_")
                ).length == 4
            )
                for (const stat in stats) stats[stat] *= 1.05;
            ...
            for (
                ...
                for (const stat in stats) stats[stat] *= 1.01;
            }
            ...
            if (
                items.armor.filter((a) =>
                    getId(a).startsWith("ELEGANT_TUXEDO_")
                ).length == 3
            )
                stats["health"] = 250;
            ...
            for (let stat in stats)
                output.weapon_stats[item.itemId][stat] = Math.max(
                    0,
                    Math.round(stats[stat])
                );

            stats.effective_health = getEffectiveHealth(
                stats.health,
                stats.defense
            );
        }
        ...
        // Apply Superior Dragon Armor full set bonus of 5% stat increase
        if (
            items.armor.filter((a) => getId(a).startsWith("SUPERIOR_DRAGON_"))
                .length == 4
        ) {
        ...
        for (const item of items.armor) {
            if (
                helper.getPath(item, "tag", "ExtraAttributes", "modifier") ==
                "renowned"
            ) {
                for (const stat in output.stats) {
        ...
        if (activePet)
            // hacky, ik; someone who is smarter than me fix this in the js correct way
        ...
        output.stats.effective_health = getEffectiveHealth(
            output.stats.health,
            output.stats.defense
        );
        ...
        for (let stat in userProfile.stats) {
            if (stat.startsWith("kills_") && userProfile.stats[stat] > 0)
                killsDeaths.push({
                    type: "kills",
                    entityId: stat.replace("kills_", ""),
                    amount: userProfile.stats[stat],
                });

            if (stat.startsWith("deaths_") && userProfile.stats[stat] > 0)
                killsDeaths.push({
                    type: "deaths",
                    entityId: stat.replace("deaths_", ""),
                    amount: userProfile.stats[stat],
                });
        }
        ...
        for (const stat of killsDeaths) {
            ...
            entityId.split("_").forEach((split, index) => {
                ...
                if (index < entityId.split("_").length - 1) entityName += " ";
            });
        ...
        if (
            "kills_guardian_emperor" in userProfile.stats ||
            "kills_skeleton_emperor" in userProfile.stats
        )
            killsDeaths.push({
                type: "kills",
                ...
                amount:
                    (userProfile.stats["kills_guardian_emperor"] || 0) +
                    (userProfile.stats["kills_skeleton_emperor"] || 0),
            });

        if (
            "kills_chicken_deep" in userProfile.stats ||
            "kills_zombie_deep" in userProfile.stats
        )
            killsDeaths.push({
                type: "kills",
                ...
                amount:
                    (userProfile.stats["kills_chicken_deep"] || 0) +
                    (userProfile.stats["kills_zombie_deep"] || 0),
            });
        ...
        output.kills = killsDeaths
            .filter((a) => a.type == "kills")
            .sort((a, b) => b.amount - a.amount);
        output.deaths = killsDeaths
            .filter((a) => a.type == "deaths")
            .sort((a, b) => b.amount - a.amount);

        const playerObject = await helper.resolveUsernameOrUuid(
            profile.uuid,
            db,
            cacheOnly
        );
        ...
        const members = await Promise.all(
            Object.keys(profile.members).map((a) =>
                helper.resolveUsernameOrUuid(a, db, cacheOnly)
            )
        );
        ...
        if (userInfo) {
            ...
            if ("emoji" in userInfo) output.display_emoji = userInfo.emoji;
        ...
        for (const sbProfile of allProfiles.filter(
            (a) => a.profile_id != profile.profile_id
        )) {
            if (
                !helper.hasPath(sbProfile, "members", profile.uuid, "last_save")
            )
                continue;
            ...
            output.profiles[sbProfile.profile_id] = {
                ...
                last_updated: {
                    ...
                    text: `last played ${moment(
                        sbProfile.members[profile.uuid].last_save
                    ).fromNow()}`,
                },
            };
        }
        ...
        output.collections = await module.exports.getCollections(
            profile.uuid,
            profile,
            cacheOnly
        );
        ...
        output.dungeons = await module.exports.getDungeons(
            userProfile,
            hypixelProfile
        );
        ...
        if ("ender_crystals_destroyed" in userProfile.stats)
            misc.dragons["ender_crystals_destroyed"] =
                userProfile.stats["ender_crystals_destroyed"];
        ...
        for (const key of burrows)
            if (key in userProfile.stats)
                misc.burrows[key.replace("mythos_burrows_", "")] = {
                    total: userProfile.stats[key],
                };

        misc.profile_upgrades = await module.exports.getProfileUpgrades(
            profile
        );
        ...
        for (const key of auctions_sell)
            if (key in userProfile.stats)
                misc.auctions_sell[key.replace("auctions_", "")] =
                    userProfile.stats[key];

        for (const key of auctions_buy)
            if (key in userProfile.stats)
                misc.auctions_buy[key.replace("auctions_", "")] =
                    userProfile.stats[key];
        ...
        for (const key in userProfile.stats)
            if (key.includes("_best_time"))
                ...
                auctions_sold[key.replace("auctions_sold_", "")] =
                    userProfile.stats[key];
            else if (key.includes("auctions_bought_"))
                auctions_bought[key.replace("auctions_bought_", "")] =
                    userProfile.stats[key];
                ...
                misc.milestones[key.replace("pet_milestone_", "")] =
                    userProfile.stats[key];
            else if (key.startsWith("mythos_burrows_dug_next_"))
                dug_next[
                    key.replace("mythos_burrows_dug_next_", "").toLowerCase()
                ] = userProfile.stats[key];
            else if (key.startsWith("mythos_burrows_dug_combat_"))
                dug_combat[
                    key.replace("mythos_burrows_dug_combat_", "").toLowerCase()
                ] = userProfile.stats[key];
            else if (key.startsWith("mythos_burrows_dug_treasure_"))
                dug_treasure[
                    key
                        .replace("mythos_burrows_dug_treasure_", "")
                        .toLowerCase()
                ] = userProfile.stats[key];
            else if (key.startsWith("mythos_burrows_chains_complete_")) {
                chains_complete[
                    key
                        .replace("mythos_burrows_chains_complete_", "")
                        .toLowerCase()
                ] = userProfile.stats[key];
            }

        for (const key in misc.dragons)
            if (misc.dragons[key] == 0) delete misc.dragons[key];

        for (const key in misc)
            if (Object.keys(misc[key]).length == 0) delete misc[key];

        for (const key in dug_next) misc.burrows.dug_next[key] = dug_next[key];
        ...
        for (const key in auctions_bought)
            misc.auctions_buy["items_bought"] =
                (misc.auctions_buy["items_bought"] || 0) + auctions_bought[key];

        for (const key in auctions_sold)
            misc.auctions_sell["items_sold"] =
                (misc.auctions_sell["items_sold"] || 0) + auctions_sold[key];
        ...
        if (diff < 3) last_updated_text = `Right now`;
        else if (diff < 60)
    ...
    getPets: async (profile) => {
        ...
        if (!helper.hasPath(profile, "pets")) return output;

        for (const pet of profile.pets) {
            if (!("tier" in pet)) continue;
            ...
            const petData = constants.pet_data[pet.type] || {
                ...
                emoji: "❓",
                head:
                    "/head/bc8ea1f51f253ff5142ca11ae45193a4ad8c3ab5e9c6eec8ba7a4fcb7bac40",
            };

            pet.texture_path = petData.head;

            let lore = [`§8${helper.capitalizeFirstLetter(petData.type)} Pet`];
            ...
            if (pet.heldItem) {
                ...
                if (heldItem in constants.pet_items) {
                    ...
                    if ("multStats" in constants.pet_items[heldItem])
                        for (const stat in constants.pet_items[heldItem]
                            .multStats)
                            if (pet.stats[stat]) {
                                pet.stats[stat] =
                                    (pet.stats[stat] || 0) *
                                    constants.pet_items[heldItem].multStats[
                                        stat
                                    ];
                            }
                }
                ...
                if (heldItemObj)
                    lore.push(
                        "",
                        `§6Held Item: §${
                            constants.tier_colors[
                                heldItemObj.tier.toLowerCase()
                            ]
                        }${heldItemObj.name}`
                    );
            ...
            if (pet.level.level < 100) {
                ...
                for (let i = 0; i < 20; i++) {
                    if (pet.level.progress > i / 20) levelBar += "§2";
                    else levelBar += "§f";
                    levelBar += "-";
                }
            ...
            lore.push(
                "",
                `§7Total XP: §e${helper.formatNumber(
                    pet.exp,
                    true,
                    10
                )} §6/ §e${helper.formatNumber(
            ...
            for (const [index, line] of lore.entries()) {
                ...
                if (index < lore.length) pet.lore += "<br>";
            }
            ...
            if (a.active === b.active)
                if (a.rarity == b.rarity) {
                    if (a.type == b.type) {
                        ...
                        let maxPetA = output
                            .filter(
                                (x) => x.type == a.type && x.rarity == a.rarity
                            )
                            .sort((x, y) => y.level.level - x.level.level);

                        maxPetA =
                            maxPetA.length > 0 ? maxPetA[0].level.level : null;

                        let maxPetB = output
                            .filter(
                                (x) => x.type == b.type && x.rarity == b.rarity
                            )
                            .sort((x, y) => y.level.level - x.level.level);

                        maxPetB =
                            maxPetB.length > 0 ? maxPetB[0].level.level : null;

                        if (maxPetA && maxPetB && maxPetA == maxPetB)
                            return a.type < b.type ? -1 : 1;
                        else return maxPetA > maxPetB ? -1 : 1;
                    }
                } else {
                    return rarity_order.indexOf(a.rarity) >
    ...
    getMissingPets: async (pets) => {
        ...
        for (const petType in constants.pet_data) {
            if (pets.map((a) => a.type).includes(petType)) continue;
            ...
            lore.forEach((line, index) => {
                ...
                if (index + 1 <= lore.length) pet.lore += "<br>";
            });
    ...
    getMissingTalismans: async (talismans) => {
        ...
        let missing = unique.filter(
            (talisman) => !talismans.includes(talisman)
        );
        ...
        missing.forEach(async (talisman) => {
            ...
            if (talisman.startsWith("WEDDING_RING_")) {
                object.texture_path =
                    "/head/8fb265c8cc6136063b4eb15450fe1fe1ab7738b0bf54d265490e1ef49da60b7c";
            ...
            if (talisman.startsWith("CAMPFIRE_TALISMAN_")) {
                object.texture_path =
                    "/head/4080bbefca87dc0f36536b6508425cfc4b95ba6e8f5e6a46ff9e9cb488a9ed";
            ...
            if (object.name == null) object.name = talisman;
            ...
            if (constants.talismans[talisman] != null) {
                ...
                if (data) {
                    object.texture_path = data.texture
                        ? "/head/" + data.texture
                        : `/item/${talisman}`;
    ...
    getTalismanCount: () => {
        ...
        for (const talisman in constants.talisman_upgrades) {
            if (talismanArray.includes(talisman)) {
                talismanArray = talismanArray.filter(
                    (name) => name !== talisman
                );
            }
        }
    ...
    getCollections: async (uuid, profile, cacheOnly = false) => {
        ...
        if (
            !("unlocked_coll_tiers" in userProfile) ||
        ...
        for (const collection of userProfile.unlocked_coll_tiers) {
            ...
            for (member in profile.members) {
                ...
                if ("collection" in memberProfile)
                    amounts.push({
                        ...
                        amount: memberProfile.collection[type] || 0,
                    });
            }
            ...
            const collectionData = constants.collection_data.filter(
                (a) => a.skyblockId == type
            )[0];
    ...
    getBagSizes: async (collections) => {
        ...
        for (const bag in constants.bag_size) {
            ...
            if (!(bagSize.collection in collections)) continue;
    ...
    getDungeons: async (userProfile, hypixelProfile) => {
        ...
        if (dungeons == null || Object.keys(dungeons).length === 0)
            return output;

        const dungeons_data = constants.dungeons;
        if (
            dungeons.dungeon_types == null ||
            Object.keys(dungeons.dungeon_types).length === 0
        )
            return output;
        ...
        for (const type of Object.keys(dungeons.dungeon_types)) {
                ...
                for (const floor of Object.keys(dungeon[key])) {
                    if (!floors[floor])
                        floors[floor] = {
                            name: `floor_${floor}`,
                            icon_texture:
                                "908fc34531f652f5be7f27e4b27429986256ac422a8fb59f6d405b5c85c76f7",
                            stats: {},
                        };
                    ...
                    if (dungeons_data.floors[id]) {
                        ...
                        if (dungeons_data.floors[id].texture)
                            floors[floor].icon_texture =
                                dungeons_data.floors[id].texture;
                    }
                    ...
                    if (key.startsWith("most_damage")) {
                        if (
                            ...
                            dungeon[key][floor] >
                                floors[floor].most_damage.value
                        )
                            floors[floor].most_damage = {
            ...
            output[type] = {
                ...
                highest_floor:
                    dungeons_data.floors[`${type}_${highest_floor}`] &&
        ...
        for (const className of Object.keys(dungeons.player_classes)) {
            ...
            if (data.experience > 0) used_classes = true;
        ...
        output.secrets_found =
            hypixelProfile.achievements.skyblock_treasure_hunter || 0;
        ...
        for (i in output.catacombs.floors) {
            ...
            if (
                data.stats.tier_completions == null ||
                data.stats.tier_completions <= 0
            )
                continue;
            ...
            for (reward_id in coll.rewards) {
                ...
                if (collections[floor_id].killed >= reward.required) {
                    ...
                    if (reward_id != "coming_soon")
                        collections[floor_id].unclaimed++;
                } else break;
        ...
        for (entry_id in journal_entries) {
            let entry = {
                name: journal_constants[entry_id]
                    ? journal_constants[entry_id].name
                    : entry_id,
                pages_collected: journal_entries[entry_id].length || 0,
                total_pages: journal_constants[entry_id]
                    ? journal_constants[entry_id].pages
                    : null,
            };
        ...
        for (let name in constants.dungeons.level_bonuses) {
            ...
            for (let x = steps[0]; x <= steps[steps.length - 1]; x += 1) {
                ...
                for (bonus in level_bonus)
                    switch (name) {
                        case "dungeon_catacombs":
                            output.catacombs.bonuses[bonus] +=
                                level_bonus[bonus];
                    }
            }
        }
    ...
    getProfileUpgrades: async (profile) => {
        const output = {};
        for (const upgrade in constants.profile_upgrades) output[upgrade] = 0;
    ...
    getProfile: async (
        ...
        if (paramPlayer.length != 32) {
            try {
                const { uuid } = await helper.resolveUsernameOrUuid(
                    paramPlayer,
                    db
                );
        ...
        if (profileObject) {
            const profileData = db.collection("profileCache").find({
                profile_id: { $in: Object.keys(profileObject.profiles) },
            });

            for await (const doc of profileData) {
                if (!helper.hasPath(doc, "members", paramPlayer)) continue;

                Object.assign(doc, profileObject.profiles[doc.profile_id]);

                allSkyBlockProfiles.push(doc);

                lastCachedSave = Math.max(
                    lastCachedSave,
                    doc.members[paramPlayer].last_save || 0
                );
            }
        } else {
        ...
        if (
            !options.cacheOnly &&
            ((Date.now() - lastCachedSave > 190 * 1000 &&
                Date.now() - lastCachedSave < 300 * 1000) ||
        ...
        if (paramProfile) {
            if (paramProfile.length == 32) {
                const filteredProfiles = allSkyBlockProfiles.filter(
                    (a) => a.profile_id.toLowerCase() == paramProfile
                );
                ...
                if (filteredProfiles.length > 0) {
                    ...
                    const profileResponse = await retry(async () => {
                        ...
                        if (!response.data.success) throw "api request failed";
                ...
                skyBlockProfiles = allSkyBlockProfiles.filter(
                    (a) => a.cute_name.toLowerCase() == paramProfile
                );
            }
        }
        ...
        for (const _profile of allSkyBlockProfiles) {
            ...
            if (response && response.request.fromCache !== true) {
                ...
                if (helper.hasPath(_profile, "community_upgrades"))
                    insertCache.community_upgrades =
                        _profile.community_upgrades;
        ...
        for (const [index, _profile] of profiles.entries()) {
            if (_profile === undefined || _profile === null) return;

            let userProfile = _profile.members[paramPlayer];

            if (
                helper.hasPath(userProfile, "last_save") &&
                userProfile.last_save > highest
            ) {
                profile = _profile;
        ...
        if (response && response.request.fromCache !== true) {
            const apisEnabled =
                ...
                Object.keys(userProfile).filter((a) =>
                    a.startsWith("experience_skill_")
                ).length > 0 &&
            ...
            if (
                options.updateArea &&
                Date.now() - userProfile.last_save < 5 * 60 * 1000
            ) {
                try {
                    ...
                    if (areaData.online && areaData.gameType == "SKYBLOCK") {
                        const areaName =
                            constants.area_names[areaData.mode] ||
                            helper.titleCase(areaData.mode.replace(/\_/g, " "));
        ...
        return {
            profile: profile,
            allProfiles: allSkyBlockProfiles,
            uuid: paramPlayer,
        };
    },

    updateLeaderboardPositions: async (db, uuid, allProfiles) => {
        if (constants.blocked_players.includes(uuid)) return;
        ...
        for (const singleProfile of allProfiles) {
            ...
            userProfile.levels = await module.exports.getLevels(
                userProfile,
                hypixelProfile
            );
            ...
            if (userProfile.hasOwnProperty("slayer_bosses")) {
                ...
                for (const mountMob in constants.mob_mounts) {
                    ...
                    for (const mount of mounts) {
                        userProfile.stats[`kills_${mountMob}`] +=
                            userProfile.stats[`kills_${mount}`] || 0;
                        userProfile.stats[`deaths_${mountMob}`] +=
                            userProfile.stats[`deaths_${mount}`] || 0;
            ...
            if (Array.isArray(userProfile.pets)) {
                for (const pet of userProfile.pets) {
                    if (!helper.hasPath(pet, "tier")) continue;

                    maxPetRarity[pet.type] = Math.max(
                        maxPetRarity[pet.type] || 0,
                        constants.pet_value[pet.tier.toLowerCase()]
                    );
                }
        ...
        values["fairy_souls"] = getMax(
            memberProfiles,
            "data",
            "fairy_souls_collected"
        );
        values["average_level"] = getMax(
            memberProfiles,
            "data",
            "levels",
            "average_level"
        );
        values["total_skill_xp"] = getMax(
            memberProfiles,
            "data",
            "levels",
            "total_skill_xp"
        ...
        for (const skill of getAllKeys(
            memberProfiles,
            "data",
            "levels",
            "levels"
        ))
            values[`skill_${skill}_xp`] = getMax(
                memberProfiles,
                "data",
                "levels",
                "levels",
        ...
        for (const slayer of getAllKeys(
            memberProfiles,
            "data",
            "slayer_bosses"
        )) {
            for (const key of getAllKeys(
                ...
                if (!key.startsWith("boss_kills_tier")) continue;
            ...
            values[`${slayer}_slayer_xp`] = getMax(
                memberProfiles,
                "data",
                "slayer_bosses",
                slayer,
        ...
        for (const item of getAllKeys(memberProfiles, "data", "collection"))
            values[`collection_${item.toLowerCase()}`] = getMax(
                memberProfiles,
                "data",
                "collection",
                item
            ...
            switch (stat) {
                ...
                default:
                    for (const floor of getAllKeys(
                        ...
                        if (
                            !constants.dungeons.floors[floor_id] ||
                            !constants.dungeons.floors[floor_id].name
                        )
                            continue;
                        ...
                        const floor_name =
                            constants.dungeons.floors[floor_id].name;
                        values[
                            `dungeons_catacombs_${floor_name}_${stat}`
                        ] = getMax(
                            memberProfiles,
                            "data",
                            "dungeons",
        ...
        for (const dungeon_class of getAllKeys(
            memberProfiles,
            "data",
            "dungeons",
            "player_classes"
        ...
        for (const singleProfile of allProfiles) {
            ...
            for (const member in singleProfile.members)
                if (
                    Array.isArray(
                        singleProfile.members[member].crafted_generators
                    )
                )
                    minionCrafts.push(
                        ...singleProfile.members[member].crafted_generators
...
async function init() {
    ...
    if (!helper.hasPath(response, "data", "collections")) return;
