...
async function getItems(base64, customTextures = false, packs, cacheOnly = false) {
  ...
  for (const item of items) {
    ...
    if (helper.hasPath(item, "id") && item.id >= 298 && item.id <= 301) {
      ...
      const type = ["leather_helmet", "leather_chestplate", "leather_leggings", "leather_boots"][item.id - 298].replace(
        "_",
        "/"
      );
...
module.exports = {
  ...
  getProfile: async (
    ...
    let profileObject = await db.collection("profileStore").findOne({ uuid: paramPlayer });
    ...
    if (response && response.request.fromCache !== true) {
      ...
      db.collection("profileStore")
        .updateOne({ uuid: paramPlayer }, { $set: insertProfileStore }, { upsert: true })
        .catch(console.error);
    }
