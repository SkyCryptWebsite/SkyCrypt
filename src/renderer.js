...
module.exports = {
    renderHead: async (url, scale) => {
        ...
        let head_bottom = resize(
            getPart(skin, 16, 0, 8, 8, 1),
            scale * (hat_factor + 0.01)
        );
        let head_top = resize(
            getPart(skin, 8, 0, 8, 8, 1),
            scale * (hat_factor + 0.01)
        );
        let head_back = flipX(
            resize(getPart(skin, 24, 8, 8, 8, 1), scale * (hat_factor + 0.01))
        );
        let head_front = resize(
            getPart(skin, 8, 8, 8, 8, 1),
            scale * (hat_factor + 0.01)
        );
        let head_left = flipX(
            resize(getPart(skin, 16, 8, 8, 8, 1), scale * (hat_factor + 0.01))
        );
        let head_right = resize(
            getPart(skin, 0, 8, 8, 8, 1),
            scale * (hat_factor + 0.01)
        );
        ...
        if (hasTransparency(getPart(skin, 32, 0, 32, 32, 1))) {
            ...
            head_back_overlay = flipX(
                resize(getPart(skin, 56, 8, 8, 8, 1), scale)
            );
            head_bottom_overlay = resize(getPart(skin, 48, 0, 8, 8, 1), scale);
            head_left_overlay = flipX(
                resize(getPart(skin, 48, 8, 8, 8, 1), scale)
            );
        ...
        if (head_top_overlay) {
            ...
            hat_bg.drawImage(
                ...
                x + y,
                z - y,
                head_left_overlay.width,
                head_left_overlay.height
            );
            ...
            if (!TALISMANS.includes(url)) {
                ...
                hat_bg.drawImage(
                    ...
                    y + x,
                    x + z,
                    head_back_overlay.width,
                    head_back_overlay.height
                );
            }
            ...
            hat_bg.drawImage(
                ...
                y - z,
                x + z,
                head_bottom_overlay.width,
                head_bottom_overlay.height
            );
            ...
            hat.drawImage(
                ...
                y - z,
                x + z,
                head_top_overlay.width,
                head_top_overlay.height + 1
            );
            ...
            hat.drawImage(
                ...
                y + x,
                x + z,
                head_front_overlay.width,
                head_front_overlay.height
            );
            ...
            hat.drawImage(
                ...
                x + y,
                z - y,
                head_right_overlay.width,
                head_right_overlay.height
            );
        }
    ...
    renderItem: async (skyblockId, query, db) => {
        ...
        if (skyblockId)
            item = Object.assign(
                item,
                await db.collection("items").findOne({ id: skyblockId })
            );
        ...
        if (query.name) {
            ...
            const filteredResults = results.filter(
                (a) => a.name.toLowerCase() == query.name.toLowerCase()
            );
        ...
        if (query.damage) item.damage = query.damage;
        ...
        if ("item_id" in item) item.id = item.item_id;
        ...
        if ("texture" in item)
            return {
                mime: "image/png",
                image: await module.exports.renderHead(
                    `http://textures.minecraft.net/texture/${item.texture}`,
                    6.4
                ),
            };
        ...
        for (const rule of itemsCss.stylesheet.rules) {
            ...
            const coords = rule.declarations[0].value
                .split(" ")
                .map((a) => Math.abs(parseInt(a)));

            outputTexture.image = await getPart(
                itemsSheet,
                ...coords,
                128,
                128,
        ...
        if (customTexture) {
            ...
            outputTexture.image = await fs.readFile(
                path.resolve(__dirname, "..", "public", customTexture.path)
            );
        }

        if (!("image" in outputTexture)) outputTexture.error = "item not found";

        return outputTexture;
    },

    init: async () => {
        itemsSheet = await loadImage(
            path.resolve(
                __dirname,
                "..",
                "public",
