...
async function main() {
    ...
    async function updateBazaar() {
        try {
            const response = await Hypixel.get("skyblock/bazaar", {
                params: { key: credentials.hypixel_api_key },
            });
            ...
            for (const productId in products) {
                ...
                await db.collection("bazaar").updateOne(
                    { productId },
                    {
                        $set: {
                            buyPrice,
                ...
                await db
                    .collection("items")
                    .updateOne({ id: productId }, { $set: { bazaar: true } });
            }
        } catch (e) {
            console.error(e);
        }
