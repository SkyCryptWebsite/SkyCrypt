...
async function main() {
    ...
    async function updateGuildLeaderboards() {
        ...
        const bar = new ProgressBar(
            "  generating guild leaderboards [:bar] :current/:total :rate guilds/s :percent :etas",
            {
                complete: "=",
                incomplete: " ",
                width: 20,
        ...
        for (const gid of guilds) {
            const guildMembers = (
                await db.collection("guildMembers").find({ gid }).toArray()
            ).map((a) => a.uuid);
            ...
            for (const key of keys) {
                ...
                if (options.mappedBy != "uuid") continue;
                ...
                for (const memberScore of memberScores) {
                    ...
                    if (isNaN(score)) {
                        if (!key.includes("best_time")) scores.push(0);
                ...
                if (key == "lb_bank") continue;

                if (scores.length < 75) continue;
