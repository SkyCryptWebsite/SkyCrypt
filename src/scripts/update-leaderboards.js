...
async function main() {
    ...
    async function updateLeaderboards() {
        ...
        for (const key of keys) {
            ...
            if (lb.sortedBy < 0) multi.zrevrange(key, 0, 49);
            else multi.zrange(key, 0, 49);
        }

        const updateUsers = _.uniq(
            (await multi.exec()).map((a) => a[1]).flat()
        );

        console.log("updating", updateUsers.length, "profiles");

        const bar = new ProgressBar(
            "  generating leaderboards [:bar] :current/:total :rate users/s :percent :etas",
            {
                complete: "=",
                incomplete: " ",
                width: 20,
